        ...
        dispatcher.register(CommandManager.literal(Uhc.MOD_ID)
                .then(CommandManager.literal("lives")
                        .then((CommandManager.argument("player", EntityArgumentType.player()
                                        ...
                                        .executes(context -> getLives(context.getSource(), EntityArgumentType.getPlayer(context, "player")))
                                        )
                                ).then(CommandManager.literal("set")
                                        .then(CommandManager.argument("lives", IntegerArgumentType.integer(0))
                                                .executes(context -> setLives(context.getSource(), EntityArgumentType.getPlayer(context, "player"), IntegerArgumentType.getInteger(context, "lives")))
                                        )
                                )
                        )
                )
