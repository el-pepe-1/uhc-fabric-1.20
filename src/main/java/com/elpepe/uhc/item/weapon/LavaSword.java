...
public class LavaSword extends SwordItem {
   public LavaSword(ToolMaterial toolMaterial, int attackDamage, float attackSpeed, Item.class_1793 settings) {
   ...
   public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
      ...
      if (feetStack.getItem().equals(ModItems.MAGMA_BOOTS) && user.isSneaking()) {
         ...
         stack.damage(3, user, (player) -> {
            player.sendToolBreakStatus(hand);
         });
         feetStack.damage(2, user, (player) -> {
