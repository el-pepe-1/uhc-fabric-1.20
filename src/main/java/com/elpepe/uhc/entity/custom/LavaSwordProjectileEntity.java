...
public class LavaSwordProjectileEntity extends MobEntity {
   ...
   public LavaSwordProjectileEntity(EntityType<? extends MobEntity> entityType, World world) {
   ...
   public static DefaultAttributeContainer.class_5133 createLavaSwordProjectileAttributes() {
      return MobEntity.createMobAttributes().add(EntityAttributes.GENERIC_MAX_HEALTH, 20.0).add(EntityAttributes.GENERIC_MOVEMENT_SPEED, 0.20000000298023224).add(EntityAttributes.GENERIC_ARMOR, 0.5).add(EntityAttributes.GENERIC_ATTACK_DAMAGE, 1.0);
   }
   ...
   public void tick() {
      ...
      if (!this.getWorld().isClient()) {
         ...
         if (this.attackCooldown <= 0) {
            List<LivingEntity> entities = this.getWorld().getEntitiesByClass(LivingEntity.class, Box.of(this.getPos(), 10.0, 10.0, 10.0), (entity) -> {
         ...
         this.getWorld().playSound((PlayerEntity)null, this.getBlockPos(), SoundEvents.BLOCK_FIRE_EXTINGUISH, SoundCategory.HOSTILE, 1.0F, 1.2F);
      }
